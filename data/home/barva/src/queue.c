 ...
 * W(n+1) = W(n) * INERTIA
 *
 * VALUES LESS THAN 0.9999 CAN CAUSE FLICKER. DEFAULT 0.9999
 */
#define INERTIA 0.9999

/*
 * When W(n) < EPSILON, the sample n is ignored (as if W(n) = 0).
 * The smaller EPSILON is, the more memory is required for queue. DEFAULT 1e-6
 */
#define EPSILON 0.5
...
struct queue init_queue(void) {
    ...
    //size_t size = (size_t)((1.0 / 60.0) * 44100.0);
    //printf("size = %ld\n", size);
